...
for (i in 1:3) {
  for (j in 1:6) {
    assign(paste(algorithms[i],number_of_variants[j],sep="_") , 
           sc_clust_data_manipulator(algorithms[i] , number_of_variants[j] , datasets_spike , results_spike_500 , spike))
  }
}
...
all_results_spike <- all_results

save(all_results_spike , file = "spike_final_results.RData")
