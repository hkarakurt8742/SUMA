...
source("functions/run_analysis_filtered.R")

########################## TABULA MURIS SPIKE #####################


load("~/Documents/R/sc_clustering/analyses/second_try/spikedata.RData")
...
load("~/Documents/R/sc_clustering/analyses/second_try/dropletdata.RData")
...
save(results_droplet_500 , results_droplet_1000 , 
     ...
     data_names_droplet , file = "droplet_results.RData")
...
load("~/Documents/R/sc_clustering/analyses/second_try/zhengmixdata.RData")
...
save(results_zhengmix_500 , results_zhengmix_1000 , 
     ...
     data_names_zheng , file = "zhengmix_results.RData")

########################## SINCELL DATASETS #####################

load("~/Documents/R/sc_clustering/analyses/second_try/sincelldata.RData")

for (i in 1:length(datasets_sincell)) {
  assign(paste(data_names_sincell[i] , "_500_results" , sep = ""),
         run_analysis_umi(datasets_sincell[[i]] , labels_sincell[[i]] , 500))
}

results_sincell_500 <- list()

for (i in 1:length(ls(pattern = "_500_results"))){
  results_sincell_500[[i]] <- get(ls(pattern = "_500_results")[i])
}
...
for (i in 1:length(datasets_sincell)) {
  assign(paste(data_names_sincell[i] , "_1000_results" , sep = ""),
         run_analysis_umi(datasets_sincell[[i]] , labels_sincell[[i]] , 1000))
}

results_sincell_1000 <- list()

for (i in 1:length(ls(pattern = "_1000_results"))){
  results_sincell_1000[[i]] <- get(ls(pattern = "_1000_results")[i])
}
...
for (i in 1:length(datasets_sincell)) {
  assign(paste(data_names_sincell[i] , "_1500_results" , sep = ""),
         run_analysis_umi(datasets_sincell[[i]] , labels_sincell[[i]] , 1500))
}

results_sincell_1500 <- list()

for (i in 1:length(ls(pattern = "_1500_results"))){
  results_sincell_1500[[i]] <- get(ls(pattern = "_1500_results")[i])
}
...
for (i in 1:length(datasets_sincell)) {
  assign(paste(data_names_sincell[i] , "_2000_results" , sep = ""),
         run_analysis_umi(datasets_sincell[[i]] , labels_sincell[[i]] , 2000))
}

results_sincell_2000 <- list()

for (i in 1:length(ls(pattern = "_2000_results"))){
  results_sincell_2000[[i]] <- get(ls(pattern = "_2000_results")[i])
}

rm(list = ls(pattern = "_2000_results"))


save(results_sincell_500 , results_sincell_1000 , 
     results_sincell_1500 , results_sincell_2000 , 
     data_names_sincell , file = "sincell_results.RData")
